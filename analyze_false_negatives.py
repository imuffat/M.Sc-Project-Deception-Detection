import argparse, os, re, numpy as np, pandas as pd, joblib
...
def extract_features(raw: str) -> dict:
    ...
    toks=s_clean.split(); n=len(toks)
    number_token_count=sum(t.isdigit() for t in toks)
    currency_count=s_basic.count("$")+s_basic.count("£")+s_basic.count("€")
    exclamation_count=s_basic.count("!"); question_count=s_basic.count("?")
    period_count=s_basic.count("."); comma_count=s_basic.count(",")
    url_count=len(re.findall(r"http[s]?://\S+|www\.\S+", s_basic))
    mention_count=s_basic.count("@"); email_count=len(re.findall(r"\b\S+@\S+\.\S+\b", s_basic))
    emoji_count=sum(1 for c in s_basic if c in "😀😁😂🤣😊😍😢😭👍🙏🔥❤️💯")
    avg_word_len=(sum(len(t) for t in toks)/n) if n else 0.0
    type_token_ratio=(len(set(toks))/n) if n else 0.0
    pronoun_ratio=(sum(t in {"i","we","you","he","she","they","me","us","him","her","them","my","our","your","his","her","their"} for t in toks)/n) if n else 0.0
    char_len=len(s_basic); uppercase_ratio=(sum(1 for c in s if c.isupper())/char_len) if char_len else 0.0
    digit_ratio=(sum(1 for c in s if c.isdigit())/char_len) if char_len else 0.0
    return {
        TEXT_COL:s_clean,"channel":"Chat","intent":"Unknown","tone":"Neutral","writing_style":"unknown",
...
def normalize_labels(series: pd.Series) -> pd.Series:
    ...
    if y.isna().any():
        ...
        raise ValueError(f"Unrecognized label values: {bad}. Provide numeric 0/1 or extend mapping.")
    return y.astype(int)

def main(data_path="train.csv", model_path="deception_gbc.joblib",
         text_col="text", label_col="label", test_size=0.2,
         threshold=0.50, out_dir="fn_analysis", explain_top=5):
    ...
    print(f"Miss rate (FNR) = {fn/(fn+tp+1e-9):.3f} ; Recall (TPR) = {tp/(tp+fn+1e-9):.3f}\n")
    ...
    fns = eval_df[(eval_df["true"]==1) & (eval_df["pred"]==0)].sort_values("proba_deceptive", ascending=False)
    fns.to_csv(os.path.join(out_dir,"false_negatives.csv"), index=False, encoding="utf-8")
    print(f"Saved {len(fns)} false negatives -> {os.path.join(out_dir,'false_negatives.csv')}")

    # Token contrasts (FN vs TP)
    ...
    from collections import Counter
    tps = eval_df[(eval_df["true"]==1) & (eval_df["pred"]==1)]
    pd.DataFrame(top_words(fns["text"]), columns=["token","count"]).to_csv(os.path.join(out_dir,"fn_top_tokens.csv"), index=False)
    pd.DataFrame(top_words(tps["text"]), columns=["token","count"]).to_csv(os.path.join(out_dir,"tp_top_tokens.csv"), index=False)

    # Optional SHAP (numeric)
    try:
        import shap
        print("\nGenerating SHAP for top false negatives (numeric features only)...")
        top_samples = fns.head(explain_top)
        ...
        for idx,row in top_samples.iterrows():
            ...
            pd.DataFrame({"feature":NUMERIC_COLS,"shap_value":vals,"value":x0[0]}).sort_values(
                "shap_value", key=np.abs, ascending=False).to_csv(os.path.join(out_dir,f"fn_{idx}_shap_numeric.csv"), index=False)
    except Exception as e:
        print("(SHAP skipped) ->", e)

if __name__ == "__main__":
    p = argparse.ArgumentParser(description="False Negative analysis for deception detector")
    ...
    p.add_argument("--out_dir", default="fn_analysis"); p.add_argument("--explain_top", type=int, default=5)
    args = p.parse_args()
